...
void	check_textures2(t_all *all, t_plr *p)
{
	...
	p->tex_x = (int)(p->wall_x * (float)(p->tex_width));
...
static void	draw_line(t_all *all, int x, int *y, int end)
{
	while ((*y) < end)
	{
...
void	paint_wall(t_all *all, int x, int y, t_plr *p)
{
	...
	all->current_color = all->params->c_celling;
	...
	all->current_color = all->params->c_floor;
	draw_line(all, x, &y, all->win->height);
	all->plr->buf[x] = all->plr->perp_wall_dist;
}
